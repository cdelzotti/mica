...
mehcached_init_header_template(struct client_state *state)
{
	...
	eth->ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4);	// ETHER_TYPE_IPv4 was renamed RTE_ETHER_TYPE_IPV4
...
mehcached_client_send_packet(struct client_state *state, struct packet_construction_state *constr_state, uint8_t port_id)
{
	...
	// s_addr/d_addr were renamed src_addr/dst_addr
	rte_memcpy(&eth->src_addr, state->client_conf->ports[port_id].mac_addr, 6);
	rte_memcpy(&eth->dst_addr, state->server_conf->ports[server_port_id].mac_addr, 6);
...
// #endif
	...
	else
	{
		...
		eth->src_addr.addr_bytes[3] = 0xff;
	}
// #ifdef MEHCACHED_MEASURE_LATENCY
// 	}
// #endif
