...
#include <rte_debug.h>
// rte_ethdev.h no longer pulls these in transitively -- struct {ether,ipv4,udp}_hdr
// moved out to lib/net and were renamed to their rte_ prefixed versions.
#include <rte_ether.h>
#include <rte_ip.h>
#include <rte_udp.h>
#include <rte_ring.h>	// for the MEHCACHED_USE_SOFT_FDIR mailbox rings
...
mehcached_remote_send_response(struct server_state *state, struct rte_mbuf *mbuf, uint8_t port_id)
{
    ...
    // struct {ether,ipv4,udp}_hdr were renamed struct rte_{ether,ipv4,udp}_hdr
    ...
    // struct ether_addr was renamed struct rte_ether_addr; s_addr/d_addr were renamed src_addr/dst_addr
    {
        struct rte_ether_addr t = eth->src_addr;
    ...
    // struct rte_mbuf no longer nests these fields under a ".pkt" sub-struct; they are direct members now
...
#ifndef NDEBUG
    // rte_mbuf_sanity_check() dropped its middle "mbuf type" argument (RTE_MBUF_PKT); only is_header remains
...
mehcached_benchmark_server_proc(void *arg)
{
    ...
    if (thread_id != thread_id % (((uint8_t)rte_lcore_count()) >> state->cpu_mode))
        return 0;
    ...
    uint64_t t_last_rx;
    uint64_t t_last_tx_flush;

    t_start = mehcached_stopwatch_now();

    t_last_rx = t_start;
    t_last_tx_flush = t_start;
...
#define USE_STAGE_GAP
    ...
    while (!exiting)
    {
        ...
        if (t_end - t_last_rx >= 1 * mehcached_stopwatch_1_usec)
        {
...
#ifdef MEHCACHED_USE_SOFT_FDIR
        {
            ...
            for (mailbox_index = 0; mailbox_index < MEHCACHED_MAX_NUMA_NODES; mailbox_index++)
                // rte_ring_sc_dequeue_burst() gained a 4th "available" out-param; NULL means "don't care", matching the old behavior
...
//#endif
        ...
        //if (packet_count < pipeline_size)
        {
            ...
            if (t_end - t_last_tx_flush >= 10 * mehcached_stopwatch_1_usec)
            {
                t_last_tx_flush = t_end;
        ...
        if ((i & 0xff) == 0)
        {
            ...
            state->num_rx_burst = num_rx_burst;
            state->num_rx_received = num_rx_received;
            state->num_tx_sent = num_tx_sent;
            state->num_tx_dropped = num_tx_dropped;
...
#ifndef NDEBUG
                        // struct rte_eth_stats.fdirmiss (a legacy Flow Director counter) was removed along with
                        // the rest of the FDIR API -- see the migration note above mehcached_set_dst_port_mask()
                        // in net_common.c. rte_flow has no equivalent aggregate "miss" counter to check here.
    ...
    while (!exiting)
    {
        ...
        for (port_id = 0; port_id < server_conf->num_ports; port_id++)
        {
            for (thread_id = 0; thread_id < ((uint8_t)rte_lcore_count()); thread_id++)
            {
                ...
                // mbuf->pkt.hash.fdir moved to mbuf->hash.fdir (the ".pkt" sub-struct is gone). Note this will
                // now read as zero regardless of steering correctness: our rte_flow rules (net_common.c) use a
                // plain QUEUE action, which does not set RTE_MBUF_F_RX_FDIR_ID/RTE_MBUF_F_RX_FDIR and populate
                // this union, unlike legacy FDIR filters. A MARK action would need to be added to the flow
                // rules to make this diagnostic meaningful again.
...
// whatever external tool generates requests must hash keys identically to this, since the
// resulting key_hash is what determines partition ownership and request routing
static
uint64_t
...
mehcached_benchmark_prepopulate_proc(void *arg)
{
    ...
    for (key_index = 0; key_index < prepopulation_conf->num_items; key_index++)
    {
        ...
        // this key encoding must match whatever external tool generates requests
...
mehcached_benchmark_server(const struct mehcached_server_partition_conf *partition_conf, const char *server_name, int cpu_mode, const char *prepopulation_filename)
{
    ...
    uint64_t port_mask = ((uint64_t)1 << rte_eth_dev_count_avail()) - 1;
    ...
    server_conf->num_ports = num_ports_max;


    // no config-supplied MAC to set any more -- query and report each port's real address instead.
    // Responses are built by mirroring whatever addresses were already in the request being answered
    // (see mehcached_remote_send_response()), so the server never needs to know its own MAC beyond this.
    printf("querying MAC address\n");
    uint8_t port_id;
    for (port_id = 0; port_id < server_conf->num_ports; port_id++)
    {
        ...
        {
            fprintf(stderr, "failed to get MAC address for port %hhu\n", port_id);
            return;
        }
        printf("port %hhu MAC: %02x:%02x:%02x:%02x:%02x:%02x\n", port_id,
...
#ifdef NETBENCH_SERVER_MEMCACHED
    {
        ...
        uint64_t t = ((uint8_t)rte_lcore_count());
        char t_str[64];
        sprintf(t_str, "%lu", t);
        printf("t = %lu\n", t);
...
#ifdef NETBENCH_SERVER_MASSTREE
    {
        ...
        masstree_t t = masstree_init(((uint8_t)rte_lcore_count()), false);
#else
        masstree_t t = masstree_init(((uint8_t)rte_lcore_count()), true);
#endif
        for (thread_id = 0; thread_id < ((uint8_t)rte_lcore_count()); thread_id++)
...
#ifdef NETBENCH_SERVER_RAMCLOUD
    {
        ramcloud_t t = ramcloud_init(((uint8_t)rte_lcore_count()));
        for (thread_id = 0; thread_id < ((uint8_t)rte_lcore_count()); thread_id++)
